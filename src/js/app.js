  ...
  data.events.forEach(event => {
    ...
    if (daysPassed < 0) {
      ...
      if (countdown <= 0) {
        ...
        eventsSection = '<section class="event event-' + event.id + ' border border-black rounded-xl px-4 py-5 flex justify-between hover:drop-shadow hover:shadow-md hover:shadow-indigo-300"><div id="eventInfo">';
      }

      if (event.link === "") {
        eventsSection += '<h4 class="mb-2 text-xl font-medium">' + event.name + '</h4>';
      } else {
        eventsSection += '<h4 class="mb-2 text-xl font-medium">' + event.name + '<a href="' + event.link + '" title="' + event.name + '" target="_blank" class="hover:text-indigo-700">' + linkIcon + '</a></h4>';
      }
      ...
      if (countdown < 0) {
        eventsSection += fireIcon + '<p class="font-bold">Ongoing!</p></div></section>';
      } else if (countdown === 0) {
        eventsSection += rocketIcon + '<p class="font-bold">Today is the day!</p></div></section>';
      } else if (countdown === 1) {
